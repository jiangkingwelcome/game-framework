                ...
                if (isWrite) {
                    ...
                    switch (type) {
                        ...
                        case "any":
                            ...
                            break;
                        case "bool":
                            // 布尔类型：支持 true/false, 1/0, "true"/"false"
                            ...
                            break;
                        case "int[]":
                            // 整数数组：支持 "1,2,3" 或 "[1,2,3]" 格式
                            if (value && value.toString().trim() !== "") {
                                let strVal = value.toString().trim();
                                if (strVal.startsWith("[")) {
                                    // JSON格式
                                    try {
                                        data[key] = JSON.parse(strVal);
                                    } catch {
                                        data[key] = [];
                                    }
                                } else {
                                    // 逗号分隔格式
                                    data[key] = strVal.split(",").map((v: string) => parseInt(v.trim())).filter((v: number) => !isNaN(v));
                                }
                            } else {
                                data[key] = [];
                            }
                        ...
                        case "string[]":
                            ...
                            if (value && value.toString().trim() !== "") {
                                ...
                                if (strVal.startsWith("[")) {
                                    ...
                                    // 逗号分隔格式
                                    data[key] = strVal.split(",").map((v: string) => v.trim());
                                }
                            } else {
                                data[key] = [];
                            }
                            ...
                            break;
                        case "float[]":
                            // 浮点数组：支持 "1.1,2.2,3.3" 或 "[1.1,2.2]" 格式
                            if (value && value.toString().trim() !== "") {
                                let strVal = value.toString().trim();
                                if (strVal.startsWith("[")) {
                                    try {
                                    ...
                                    data[key] = strVal.split(",").map((v: string) => parseFloat(v.trim())).filter((v: number) => !isNaN(v));
                                }
                            } else {
                                data[key] = [];
                            }
